<?php 
   ...
   $crud = new Crud();
   $result = $crud->getBarangMasuk();

   if(isset($_POST['search'])){
      if($crud->searchBarangMasuk() == true){
         $result = $crud->searchBarangMasuk();
      }else{
         echo "
				<script>
					alert('Data tidak ditemukan');
					document.location.href = 'index.php?page=barangmasuk';
				</script>
			";
      }
   }
...
<div class="row mt-4">
   <div class="col">
      <a href="index.php?page=create-bm" class="btn btn-primary btn-sm"><i class="fas fa-plus mr-2"></i>Tambah Barang Masuk</a>
   </div>
   <div class="col">
...
<table class="table table-bordered mt-3">
   ...
   <tbody>
      ...
      <tr>
         ...
         <td><?= $r['tanggal_bm'] ?></td>
         <td>
            <a href="index.php?id=<?= $r['id'] ?>&page=update-bm" class="btn btn-warning btn-sm text-light"><i class="fas fa-edit"></i></a>
            <a href="index.php?id=<?= $r['id'] ?>&page=delete-bm" class="btn btn-danger btn-sm text-light"><i class="fas fa-trash"></i></a>
         </td>
      </tr>
      <?php endforeach ?>
   </tbody>
